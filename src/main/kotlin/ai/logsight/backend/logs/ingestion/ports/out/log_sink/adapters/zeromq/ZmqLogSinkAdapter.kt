...
import ai.logsight.backend.connectors.log_sink.zeromq.ZeroMqConnector
...
class ZmqLogSinkAdapter(
    val zeroMqConnector: ZeroMqConnector,
    val jsonLogBatchSerializer: LogBatchSerializer,
) : LogSinkAdapter {
    private val logger = LoggerImpl(ZmqLogSinkAdapter::class.java)
    ...
    override fun sendBatch(logBatchDTO: LogBatchDTO) {
        ...
        if (!zeroMqConnector.send(logBatchDTOJsonString)) {
