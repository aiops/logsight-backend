...
class UserStorageImpl(
    ...
    override fun changePassword(changePasswordCommand: ChangePasswordCommand): User {
        val userEntity = userRepository.findByEmail(changePasswordCommand.email).orElseThrow { UserNotFoundException() }

        if (changePasswordCommand.newPassword != changePasswordCommand.confirmNewPassword) throw PasswordsNotMatchException()

        // change password
        userEntity.password = passwordEncoder.encode(changePasswordCommand.newPassword)
        // save changes
        return userRepository.save(userEntity).toUser()
    }
