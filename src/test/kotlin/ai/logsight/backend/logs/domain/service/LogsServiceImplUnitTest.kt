...
class LogsServiceImplUnitTest {
    ...
    @InjectMocks
    private lateinit var logsServiceImpl: LogsServiceImpl
    ...
    fun `should return valid log receipt`() {
        ...
        // when
        val logsReceipt = logsServiceImpl.processLogBatch(logBatchDTOAppReady)
    ...
    fun `should throw exception due to invalid app state creating`() {
        val exception = assertFailsWith<ApplicationStatusException> {
            logsServiceImpl.processLogBatch(logBatchDTOAppCreating)
        }
    ...
    fun `should throw exception due to invalid app state deleting`() {
        val exception = assertFailsWith<ApplicationStatusException> {
            logsServiceImpl.processLogBatch(logBatchDTOAppDeleting)
        }
        assertNotNull(exception)
    }
    ...
    fun `should throw an runtime exceptions if logs receipt storage returns null`() {
        ...
        // when
        val exception = assertFailsWith<RuntimeException> { logsServiceImpl.processLogBatch(logBatchDTOAppReady) }
    ...
    fun `should forward internal exceptions from the logs receipt update`() {
        ...
        // when
        val exception = assertFailsWith<Exception> { logsServiceImpl.processLogBatch(logBatchDTOAppReady) }
